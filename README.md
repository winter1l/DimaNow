...
공지·자료·과제처럼 본문이 있는 항목은 외부 브라우저를 열지 않고 앱의 네이티브 상세 화면에서 실행 가능한 태그와 이벤트 속성을 제거해 표시합니다. 공식 게시판의 `table_view_basic` 본문과 `fncFileDown` 첨부파일도 같은 인증 세션으로 처리합니다. 첨부파일은 임시 `.part` 파일에 내려받은 뒤 사용자가 선택한 Android 문서 위치로만 복사하며, 광범위한 저장소 권한을 요구하지 않습니다. 영상 콘텐츠처럼 독립된 글 본문이 없는 항목은 네이티브 글로 가장하지 않고, 별도 외부 브라우저 세션을 만들지 않는 앱 내부 공식 LMS 강의 화면으로 연결합니다. 상세를 열 때 서버를 다시 확인해 첨부파일이 추가·삭제·변경됐는지 알리고, 연결할 수 없으면 마지막으로 저장한 상세를 명시해 표시합니다. 설정의 `LMS 계정`에서 저장 계정, 세션과 별도 `lms-cache.db`를 함께 삭제할 수 있습니다.
