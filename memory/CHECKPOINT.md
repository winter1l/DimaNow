...
Next action: if exact physical Galaxy serial `test-device-private` appears in `adb devices -l`, use `adb -s test-device-private install -r` and inspect cold-launch PID/FATAL/ANR. Do not claim production Samsung Now Bar/AOD acceptance absent fresh direct physical observation.
