...
Next action: commit/publish is NOT done - the working tree holds both the D-043 LMS work and this D-044 pass; version 1.4 (5) is now shared by three distinct artifacts, so bump versionCode/versionName before the next release per the D-040 update rule. Live authenticated LMS parsing remains unverified (no credentials entered).
