...
목록은 LMS의 `전체 학습` 화면을 기준으로 한 번에 동기화하고 5분 동안 기기 캐시를 사용합니다. 기본 `오늘` 보기는 KST 기준 새 소식, 기한이 지난 미완료 항목, 오늘부터 3일 뒤까지의 일정과 접힌 완료 항목을 묶어 표시합니다. `전체` 보기에서는 기존 과목·유형·읽음 필터와 전체 기록을 그대로 사용할 수 있습니다. LMS가 제공한 완료·미완료 탭을 확인한 경우에만 상태 배지를 표시하며, 선택 상태 요청이 실패해도 전체 목록과 직전 상태는 보존합니다. 새 항목과 제목·마감·링크 변경은 열기 전까지 각각 `새 항목`, `변경됨`으로 표시됩니다.

항목은 외부 브라우저를 열지 않고 앱 내부 상세 화면에서 실행 가능한 태그와 이벤트 속성을 제거해 표시합니다. 목록을 거쳐야 하는 유형은 해당 항목의 내부 링크를 한 번 더 확인하고, 별도 링크가 없는 LMS 형식은 선택한 행만 표시합니다. 상세를 열 때 서버를 다시 확인해 첨부파일이 추가·삭제·변경됐는지 알리고, 연결할 수 없으면 마지막으로 저장한 상세를 명시해 표시합니다. 첨부파일은 인증된 세션으로 임시 `.part` 파일에 내려받은 뒤 사용자가 선택한 Android 문서 위치로만 복사하며, 광범위한 저장소 권한을 요구하지 않습니다. 설정의 `LMS 계정`에서 저장 계정, 세션과 별도 `lms-cache.db`를 함께 삭제할 수 있습니다.
