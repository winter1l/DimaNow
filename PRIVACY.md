...
앱은 `winter1l.github.io`의 정적 JSON과 GitHub 공개 Releases API를 확인하고, 사용자가 원문 버튼을 누르면 Android 브라우저로 DIMA 또는 Instagram 공개 페이지를 엽니다. 수업 탭을 사용하면 공식 `lms.dima.ac.kr`과 통합 인증의 `portal.dima.ac.kr`에 로그인 정보와 세션 쿠키가 전송됩니다. 자동 로그인과 수업 동기화는 앱이 화면에 있을 때만 동작하며 백그라운드 작업으로 계정에 접속하지 않습니다. 업데이트 다운로드는 사용자가 직접 요청한 경우에만 GitHub release asset에서 실행됩니다. 기숙사 사진을 올릴 때는 Cloudflare Worker가 요청 IP를 중복 제한용 단방향 해시로 최대 10분 처리합니다. 셔틀 미도착 신고에는 서비스 날짜·시간표 revision·물리 운행/정류장 식별자와 무작위 설치 토큰만 전송합니다. 서버는 토큰의 HMAC만 최대 7일 보관하며 GPS 좌표, 계정, 시간표 또는 원시 토큰을 저장하지 않습니다. GitHub Pages, GitHub API, Cloudflare와 외부 브라우저 서비스에는 해당 서비스의 접속 기록 정책이 적용될 수 있습니다.
