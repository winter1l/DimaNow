...
The closest upcoming shuttle uses the solid primary color in light mode, or the solid error color when it is the last departure; later departures keep the softer containers. Directional tab slides and staggered card translation remain, but surface alpha animation is removed so card elevation does not disappear in intermediate frames. A newly combined runtime snapshot is stored before side effects, and a changed effective zone immediately invalidates both shuttle-bearing widgets and restores the shared minute alarm through the normal widget render path.
